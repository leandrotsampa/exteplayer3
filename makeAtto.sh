...
function usage {
   ...
   echo "platform:       mipsel | mipsel_softfpu | armv7 | armv7ve | armv5t"
   echo "ffmpeg_ver:     ignore | 2.8.5 | 3.0.5 | 3.1.1 | 3.2.2"
   exit 1
}
...
if [ "$EPLATFORM" != "mipsel" -a "$EPLATFORM" != "mipsel_softfpu" -a "$EPLATFORM" != "armv7" -a "$EPLATFORM" != "armv7ve" -a "$EPLATFORM" != "armv5t" ];
then
    echo "Please give supported platform (mipsel|mipsel_softfpu|armv7|armv5t) version!"
    usage
fi

if [ "$FFMPEG_VERSION" != "ignore" -a "$FFMPEG_VERSION" != "2.8.5" -a "$FFMPEG_VERSION" != "3.0.5" -a "$FFMPEG_VERSION" != "3.1.1" -a "$FFMPEG_VERSION" != "3.2.2" ];
then
...
case "$EPLATFORM" in
    ...
    armv7ve)
        ...
        CFLAGS=" -march=armv7ve -mtune=cortex-a53 -mfpu=neon-vfpv4 -mvectorize-with-neon-quad "
        ;;
    armv5t)
...
EXTEPLAYER3_OUT_FILE=exteplayer3
...
"$CROSS_COMPILE"gcc -fdata-sections -ffunction-sections -Wl,--gc-sections -Os $CFLAGS --sysroot=$SYSROOT $LDFLAGS $CPPFLAGS \
-I"$CURR_PATH"/external -I"$CURR_PATH"/include  -I$FFMPEG_PATH/usr/include/ -L$FFMPEG_PATH/usr/lib/ $SOURCE_FILES -o $EXTEPLAYER3_OUT_FILE \
-Wfatal-errors -lc -lm -lpthread -lavformat -lavcodec -lavutil -lswresample 
